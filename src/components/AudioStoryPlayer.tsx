...
export const AudioStoryPlayer: React.FC<Props> = ({ title, durationSeconds, narrator, script, highlights, placeName, audioUrl, isDemo, isAiGenerated }) => {
  ...
  return <div className={`sira-audio rounded-2xl bg-[#160E36] border p-5 ${playing ? 'border-[#E5C158] shadow-lg shadow-[#D4AF37]/10' : 'border-[#D4AF37]/30'}`}>
    ...
    <div className="flex items-center justify-between gap-2">
      ...
      <button onClick={reset} aria-label="إعادة الصوت إلى البداية" className="min-w-10 min-h-10 p-2 text-[#E5C158]"><RotateCcw size={18} /></button>
      {audioUrl && <button aria-label="تغيير سرعة الصوت" className="min-h-10 rounded-lg border border-[#D4AF37]/30 px-2.5 py-1.5 text-xs text-[#E5C158]" onClick={() => { const next = rate === 1 ? 1.25 : rate === 1.25 ? 1.5 : 1; setRate(next); if (audio.current) audio.current.playbackRate = next; }}>{rate}×</button>}
    </div>
