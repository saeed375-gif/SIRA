...
interface Props {
  ...
  audioUrl?: string; isDemo?: boolean;
}
const format = (n: number) => `${Math.floor(n / 60)}:${Math.floor(n % 60).toString().padStart(2, '0')}`;
export const AudioStoryPlayer: React.FC<Props> = ({ title, durationSeconds, narrator, script, highlights, placeName, audioUrl, isDemo }) => {
  ...
  return <div className={`sira-audio rounded-2xl bg-[#160E36] border p-5 ${playing ? 'border-[#E5C158] shadow-lg shadow-[#D4AF37]/10' : 'border-[#D4AF37]/30'}`}>
    <div className="flex gap-3 items-start"><Volume2 className="w-5 h-5 text-[#E5C158] shrink-0" /><div>
      <p className="text-xs text-[#E5C158] mb-1">{placeName} · {audioUrl ? (isDemo ? 'عيّنة صوتية مصنوعة' : narrator) : 'قراءة آلية للنص'}</p>
      <h3 className="text-lg text-[#FAF8F5] font-bold">{title}</h3>
    </div></div>
    {audioUrl && <audio ref={audio} src={audioUrl} preload="none" onPlay={() => { setPlaying(true); setEnded(false); }} onPause={() => setPlaying(false)} onEnded={() => { setPlaying(false); setEnded(true); }} onTimeUpdate={e => setTime(e.currentTarget.currentTime)} onLoadedMetadata={e => setDuration(e.currentTarget.duration)} onError={() => { setPlaying(false); setError('تعذّر تحميل الملف الصوتي.'); }} />}
    ...
    <div className="flex items-center justify-between gap-2">
      ...
      {audioUrl && <button aria-label="تغيير سرعة الصوت" className="text-xs text-[#E5C158]" onClick={() => { const next = rate === 1 ? 1.25 : rate === 1.25 ? 1.5 : 1; setRate(next); if (audio.current) audio.current.playbackRate = next; }}>{rate}×</button>}
    </div>
    {error && <p role="alert" className="text-sm text-[#FFE79A] mt-3">{error}</p>}
    <details className="mt-4 text-xs text-[#C4B7D8]"><summary className="cursor-pointer py-2">{audioUrl ? 'وصف الصوت ومصدره' : 'اقرأ النص'}</summary><p className="leading-loose mt-2">{script || highlights[0]?.text}</p>{isDemo && <p className="mt-2">صوت تجريبي مصنوع؛ ليس تسجيلًا ميدانيًا من القدس.</p>}</details>
  </div>;
};
